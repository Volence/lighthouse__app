...
exports.runAudits = async (req, res) => {
    try {
        await consoleErrorController.runAllAudits();
        let sites = await Site.find()
        await utils.asyncForEach(sites, async (siteData) => {
            try {
...
exports.runLighthouseRequest = async (req, res) => {
    try {
        let queryParam = {};
        queryParam.siteName = req.body.siteName;
        const siteData = await Site.findOne(queryParam);
        await runLighthouseAndSaveToDatabase(siteData, req.body.siteName)
        res.send({complete: 'done'});
    } catch(err) {
        res.send(err);
    }
...
const runLighthouseAndSaveToDatabase = (siteData, siteName) => {
    let time = moment.utc();
...
const launchChromeAndRunLighthouse = (url, flags = {chromeFlags: ['--headless', '--disable-gpu', '--no-sandbox']}, config = null) => {
    return new Promise( async (res, rej) => {
        try {
...
const formatResults = (results) => { 
    resultsFormatted = {}
    let {categories: {performance: {score: performanceScore}, seo: {score: seoScore}, accessibility: {score: accessibilityScore}, 'best-practices': {score: bestPracticesScore}}} = results;
    resultsFormatted = {...resultsFormatted, ...formatListOfAudits(results)}
    resultsFormatted = formatAuditResults(resultsFormatted, results);
    resultsFormatted.scores = {performance: performanceScore, bestPractice: bestPracticesScore, accessibility: accessibilityScore, seo: seoScore};
